   ...
   amount: Yup.number().test('check-amount', '', function(value, {createError, path}) {
     	...
     	if ((currency === 'USD' && value < 200) || 
      	(currency === 'GTQ' && value < 150000)) {
     		return createError({
            message: `Minimun Withdraw of ${currency} ${currency == 'USD' ? 200 : 1500}`,
            path
			});
  		}
      	...
      	(currency === 'GTQ' && value > 160000)) {
     		return createError({
            message: `Maximum Withdraw of ${currency} ${currency == 'USD' ? 20000 : 150000}`,
            path
			});
  		}
...
export const WithdrawForm = ({onSubmit, onError, ...props}) => {
